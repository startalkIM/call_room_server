...
public class RoomJsonRpcHandler extends DefaultJsonRpcHandler<JsonObject> {
  ...
  public final void handleRequest(Transaction transaction, Request<JsonObject> request)
      throws Exception {
    String ckey = JsonRpcUserControl.getStringParam(request, ProtocolElements.COMMON_PARAMETERS_CKEY);
    if(cKeyChecker.checkCKEY(ckey)==null)
    {
      throw new RuntimeException("valid ckey!");
    }
    ...
    log.error("Session #{} - request: {}", sessionId, request);

    notificationService.addTransaction(transaction, request);

    String roomId = userControl.getParticipantSession(transaction).getRoomName();

    ParticipantRequest participantRequest = new ParticipantRequest(sessionId,
            Integer.toString(request.getId()),roomId);
  ...
  public final void afterConnectionClosed(Session session, String status) throws Exception {
    ...
    ParticipantRequest preq = new ParticipantRequest(sid, null, null);
