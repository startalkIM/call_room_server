...
public class JsonRpcUserControl {
  ...
  public void joinRoom(Transaction transaction, Request<JsonObject> request,
    ...
    String topic = getStringParam(request,ProtocolElements.JOINROOM_TOPIC_PARAM);
    long startTime = getLongParam(request,ProtocolElements.JOINROOM_STARTTIME_PARAM);
    int plat = 0;
    if (request.getParams().has(ProtocolElements.JOINROOM_PLAT_PARAM)) {
      plat = getIntParam(request,ProtocolElements.JOINROOM_PLAT_PARAM);
    }
    participantRequest.setRoomId(roomName);
    if(System.currentTimeMillis() - startTime > 1000*60*60*24) {
      throw new RuntimeException("To exceed the time limit");
    }
    ...
    roomManager.joinRoom(userName, roomName,topic,plat, dataChannels, true, participantRequest);
  }
  ...
  public void leaveRoomAfterConnClosed(String sessionId) {
    try {
      roomManager.evictParticipant(sessionId,null);
  ...
  public void leaveRoom(Transaction transaction, Request<JsonObject> request,
    ...
    String roomId = null;
    if (transaction != null) {
      roomId = getParticipantSession(transaction).getRoomName();
    }
    if (roomId == null) { // null when afterConnectionClosed
      ...
      if (exists) {
        log.debug("Participant with sessionId {} is leaving room {}", pid, roomId);
        roomManager.leaveRoom(participantRequest);
        log.info("Participant with sessionId {} has left room {}", pid, roomId);
      } else {
        log.warn("Participant with session Id {} not found in room {}. "
            + "Using the admin method to evict the user.", pid, roomId);
        leaveRoomAfterConnClosed(pid);
      }
    }
  }
