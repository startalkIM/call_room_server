...
public class ShardedJedisSentinelPool extends Pool<ShardedJedisSentinelPool.MyShardedJedis> {
	...
	private List<JedisShardInfo> makeShardInfoList(List<HostAndPort> masters) {
		...
		for (HostAndPort master : masters) {
			String uri = "redis://"+master.getHost()+":"+master.getPort()+"/"+this.database;
    ...
    public static class MyShardedJedis extends ShardedJedis
	{

		public MyShardedJedis(List<JedisShardInfo> shards, Hashing algo, Pattern keyTagPattern) {
     ...
     */
    protected static class ShardedJedisFactory implements PooledObjectFactory<MyShardedJedis> {
    ...
    protected class MyJedisShardInfo extends JedisShardInfo
	{

		public MyJedisShardInfo(String host, String name) {
			super(host, name);
		}
