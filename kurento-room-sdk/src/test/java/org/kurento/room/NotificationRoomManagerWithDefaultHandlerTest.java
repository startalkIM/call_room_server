...
public class NotificationRoomManagerWithDefaultHandlerTest {
  ...
  public void setup() {
    ...
    for (int i = 0; i < USERS; i++) {
      ...
      usersParticipantRequests.put(users[i], new ParticipantRequest(users[i], requestIds[i],roomx));
  ...
  public void joinNewRoom() {
    final ParticipantRequest participantRequest = new ParticipantRequest(userx, requestIdx,roomx);
  ...
  public void joinManyUsersManyRooms() {
    ...
    for (Entry<String, String> userRoom : usersRooms.entrySet()) {
      manager.joinRoom(userRoom.getKey(), userRoom.getValue(),"test", 0,false, true,
  ...
  public void leaveRoom() {
    ...
    final ParticipantRequest participantRequestX = new ParticipantRequest(userx, requestIdx,roomx);
  ...
  public void manyPublishers() {
    ...
    for (UserParticipant userParticipant : usersParticipants.values()) {
      ...
      assertThat(manager.getPeerPublishers(userParticipant.getParticipantId(),roomx), equalTo(allOthers));
      assertThat(manager.getPeerSubscribers(userParticipant.getParticipantId(),roomx), equalTo(allOthers));
    }
  ...
  private void userJoinRoom(final String room, String user,
    ...
    manager.joinRoom(user, room, "test",0,false, true, participantRequest);
