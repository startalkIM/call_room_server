...
public class RoomManagerTest {
  ...
  public void joinManyUsersManyRooms() {
    ...
    for (Entry<String, String> userRoom : usersRooms.entrySet()) {
      ...
      Set<UserParticipant> peers = manager.joinRoom(user, room, room,0,false, true,
  ...
  public void leaveRoom() {
    ...
    Set<UserParticipant> remainingUsers = manager.leaveRoom(pidx,roomx);
  ...
  public void publisherLifecycle() {
    ...
    assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
        manager.publishMedia(participantId0, roomx,true, SDP_WEB_OFFER, false));
    ...
    for (String pid : usersParticipantIds.values()) {
      if (!pid.equals(participantId0)) {
        assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
            manager.subscribe(users[0],roomx, SDP_WEB_OFFER, pid));
      }
    }
    assertThat(manager.getSubscribers(roomx).size(), is(users.length - 1));

    manager.unpublishMedia(participantId0,roomx);
  ...
  public void rtpPublisherLifecycle() {
    ...
    assertEquals("SDP RTP answer doesn't match", SDP_RTP_ANSWER,
        manager.publishMedia(pidx,roomx, true, SDP_RTP_OFFER, false));

    assertThat(manager.getPublishers(roomx).size(), is(1));

    for (String pid : usersParticipantIds.values()) {
      assertEquals("SDP WEB answer (for the web peer) doesn't match", SDP_WEB_ANSWER,
          manager.subscribe(userx,roomx, SDP_WEB_OFFER, pid));
    }
    assertThat(manager.getSubscribers(roomx).size(), is(users.length));

    manager.unpublishMedia(pidx,roomx);
    assertThat(manager.getPublishers(roomx).size(), is(0));
  ...
  public void invertedPublisherLifecycle() {
    ...
    assertEquals("SDP server offer doesn't match", SDP_WEB_SERVER_OFFER,
        manager.generatePublishOffer(participantId0,roomx));

    assertThat(manager.getPublishers(roomx).size(), is(0));

    assertEquals("SDP updated offer doesn't match", SDP_WEB_SERVER_UPDATED_OFFER,
        manager.publishMedia(participantId0,roomx, false, SDP_WEB_PEER_ANSWER, false));
  ...
  public void publishAndLeave() {
    ...
    Set<UserParticipant> remainingUsers = manager.leaveRoom(participantId0,roomx);
  ...
  public void publishWithLoopbackError() {
    ...
    assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
        manager.publishMedia(participantId0,roomx, true, SDP_WEB_OFFER, true));
  ...
  public void invertedPublishWithLoopback() {
    ...
    assertEquals("SDP updated offer doesn't match", SDP_WEB_SERVER_UPDATED_OFFER,
        manager.publishMedia(participantId0,roomx, false, SDP_WEB_PEER_ANSWER, true));
  ...
  public void publishWithAlternativeLoopbackSrc() {
    ...
    assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
        manager.publishMedia(participantId0,roomx, true, SDP_WEB_OFFER, hb, null, true));
  ...
  public void publishWithAlternativeLoopbackSrcAudioType() {
    ...
    assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
        manager.publishMedia(participantId0,roomx, true, SDP_WEB_OFFER, hb, MediaType.AUDIO, true));
  ...
  public void muteUnmutePublished() {
    ...
    manager.mutePublishedMedia(MutedMediaType.ALL, participantId0,roomx);

    // disconnects once from the PassThrough
    verify(endpoint).disconnect(passThru, webRtcDisconnectCaptor.getValue());

    manager.unmutePublishedMedia(participantId0,roomx);
  ...
  public void muteUnmuteSubscribed() {
    ...
    manager.muteSubscribedMedia(users[0], MutedMediaType.ALL, participantId1,roomx);
    ...
    manager.unmuteSubscribedMedia(users[0],roomx, participantId1);
  ...
  public void addMediaFilterInParallel() throws InterruptedException, ExecutionException {
    ...
    exec.submit(new Callable<Void>() {
      ...
      public Void call() throws Exception {
        ...
        manager.addMediaElement(participantId0, filter,roomx);
        return null;
      }
    });
  ...
  public void addMediaFilterBeforePublishing() throws InterruptedException, ExecutionException {
    ...
    for (String pid : usersParticipantIds.values()) {
      if (!pid.equals(participantId0)) {
        assertEquals("SDP answer doesn't match", SDP_WEB_ANSWER,
            manager.subscribe(users[0], SDP_WEB_OFFER, pid,roomx));
      }
    }
    assertThat(manager.getSubscribers(roomx).size(), is(users.length - 1));
  ...
  private Set<UserParticipant> userJoinRoom(final String room, String user, String pid,
    ...
    Set<UserParticipant> existingPeers = manager.joinRoom(user, room,room,0, false, webParticipant, kcsi,
        pid);
