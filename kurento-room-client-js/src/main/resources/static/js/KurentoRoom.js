        ...
        kurento.sendRequest('joinRoom', joinParams, function (error, response) {
            if (error) {
                ...
                for (var i = 0; i < length; i++) {
                    var participantOptions = exParticipants[i];
                    participantOptions.configuration = configuration;
...
function Stream(kurento, local, room, options) {
    ...
    this.playThumbnail = function (thumbnailId) {
        ...
        name.appendChild(document.createTextNode(that.getNick()));
        name.id = "name-" + that.getPID();
        name.className = "name";
    ...
    this.init = function () {
        ...
        this.getMedia(constraints,true);
    }

    this.getMedia = function (constraints){
         navigator.mediaDevices.getUserMedia(constraints)
                .then(function (userStream) {
                    ...
                    if(constraints.video)
                    {
                         ...
                         that.getMedia(constraints,init);
                    }
                    else{
        ...
        if (local) {
        	 var options = {
        			videoStream: wrStream,
             		...
             		peerConnection:that.initPc(configuration)
             }
        	 if (dataChannel) {
    ...
    this.initPc = function(pcConfig){
        pcConfig.iceTransportPolicy='all';
        pcConfig.rtcpMuxPolicy='require';
        pcConfig.tcpCandidatePolicy='disabled';
        pcConfig.bundlePolicy='max-bundle';
        pcConfig.continualGatheringPolicy='gatherContinually';
        pcConfig.keyType='ecdsa';
        var pcConstraints={optional:[{'googIPv6': true},{'DtlsSrtpKeyAgreement':true}]};
