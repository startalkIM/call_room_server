 ...
 */
public class PCJsonRpcUserControl extends JsonRpcUserControl {
  ...
  private static final Logger log = LoggerFactory.getLogger(PCJsonRpcUserControl.class);
  ...
  public PCJsonRpcUserControl(NotificationRoomManager roomManager) {
    super(roomManager);
  }
  ...
  public void customRequest(Transaction transaction, Request<JsonObject> request,
    ...
    try {
      ...
      String roomId = participantRequest.getRoomId();
      if (hatOn) {
        ...
        FaceOverlayFilter faceOverlayFilter = new FaceOverlayFilter.Builder(
            roomManager.getPipeline(pid,roomId)).build();
        ...
        roomManager.addMediaElement(pid,roomId, faceOverlayFilter);
        ...
        roomManager.removeMediaElement(pid,roomId, (MediaElement) transaction.getSession().getAttributes()
