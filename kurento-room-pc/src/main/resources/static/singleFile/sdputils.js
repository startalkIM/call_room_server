...
function maybeSetAudioSendBitRate(sdp, params) {
  ...
  logUtil('Prefer audio send bitrate: ' + params.audioSendBitrate);
...
function maybeSetAudioReceiveBitRate(sdp, params) {
  ...
  logUtil('Prefer audio receive bitrate: ' + params.audioRecvBitrate);
...
function maybeSetVideoSendBitRate(sdp, params) {
  ...
  logUtil('Prefer video send bitrate: ' + params.videoSendBitrate);
...
function maybeSetVideoReceiveBitRate(sdp, params) {
  ...
  logUtil('Prefer video receive bitrate: ' + params.videoRecvBitrate);
...
function preferBitRate(sdp, bitrate, mediaType) {
  ...
  if (mLineIndex === null) {
    logUtil('Failed to add bandwidth line to sdp, as no m-line found');
    return sdp;
  }
  ...
  var cLineIndex = findLineInRange(sdpLines, mLineIndex + 1,
      nextMLineIndex, 'c=');
  if (cLineIndex === null) {
    logUtil('Failed to add bandwidth line to sdp, as no c-line found');
    return sdp;
  }
...
function maybeSetVideoSendInitialBitRate(sdp, params) {
  ...
  if (bitrate) {
    if (initialBitrate > bitrate) {
      logUtil('Clamping initial bitrate to max bitrate of ' +
                   bitrate + ' kbps.');
  ...
  if (mLineIndex === null) {
    logUtil('Failed to find video m-line');
    return sdp;
  }
...
function maybePreferCodec(sdp, type, dir, codec) {
  ...
  if (!codec) {
    logUtil('No preference on ' + str + '.');
    return sdp;
  }

  logUtil('Prefer ' + str + ': ' + codec);
