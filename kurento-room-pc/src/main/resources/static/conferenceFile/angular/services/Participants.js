...
function AppParticipant(stream) {
    ...
    function selectIcon(plat) {
        switch (plat) {
            case 0: //pc
                return 'md-laptop-windows';
            case 1: //mac
                return 'md-laptop-mac';
            case 2: //android
                return 'md-android';
            case 3: //iOS
                return 'md-phone-iphone';
            case 4: // linux
                return 'md-laptop-windows';
            case 5: // web
                return 'md-web';
            default:
                return 'md-desktop-windows';
        }
    }

    function selectColor(plat) {
        switch (plat) {
            case 0: //pc
                return 'btn--orange';
            case 1: //mac
    ...
    function playVideo() {
        ...
        that.videoElement.className = "video swiper-slide";

        var buttonVideo = document.createElement('button');
        var iconClass = selectIcon(that.stream.getPlat());
        var color = selectColor(that.stream.getPlat());
        buttonVideo.className = 'action btn btn--m ' + color + ' btn--fab mdi ' + iconClass;
        ...
        document.getElementById("participants-swiper-wrapper").appendChild(that.videoElement);
    ...
    if (CLIENT_API == null) {
        ...
        var pid = stream.getPID();
        var nike = stream.getNick();

        var requestParam = {};
        var user = {};
        user.username = pid.split("@")[0];
        user.host = pid.split("@")[1];
        requestParam.system = "qtalk";
        requestParam.users = new Array();
        requestParam.users[0] = user;
        ...
        var url = pathInfo + "getUserNickByUserName";
        $.ajax({
            "url": url,
        ...
        if (CLIENT_API.getNickByUserId) CLIENT_API.getNickByUserId(stream.getPID());
    }
...
function Participants() {
    ...
    function updateVideoStyle() {
        var _WIDTH = ($(window).width() - 375) / 150 > 11 ? 11 : ($(window).width() - 375) / 150
        var MAX_WIDTH = 25 - _WIDTH;
    ...
    this.showError = function ($window, LxNotificationService, e) {
        ...
        LxNotificationService.alert('错误!', e.error.message, '关闭', function (answer) {
            displayingRelogin = false;
            if (CLIENT_API == null) {
                if (COOKIE_PARAMETER.plat == 2) {
                    $window.location.href = "qim://close";
                } else {
                    $window.location.href = "#/call?status=hangup";
                }
            } else {
                CLIENT_API.close_video_room(roomName);
            }
        });
    };

    this.forceClose = function ($window, LxNotificationService, msg, roomName) {
        if (displayingRelogin) {
            console.warn('Already displaying an alert that leads to relogin');
            return false;
        }
        displayingRelogin = true;
        that.removeParticipants();
        LxNotificationService.alert('警告!', msg, '关闭', function (answer) {
            displayingRelogin = false;
    ...
    this.showTips = function ($window, LxNotificationService, msg, roomName) {
        if (displayingRelogin) {
            console.warn('Already displaying an alert that leads to relogin');
            return false;
        }
        displayingRelogin = true;
        that.removeParticipants();
        LxNotificationService.alert('提示!', msg, '知道了', function (answer) {
            displayingRelogin = false;
        });
    };

    this.alertMediaError = function ($window, LxNotificationService, roomName, msg, callback) {
        if (displayingRelogin) {
            console.warn('Already displaying an alert that leads to relogin');
            return false;
        }
